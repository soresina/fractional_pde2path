...
kv=0:0.01:1.5; kl=length(kv); muv=zeros(2,kl); % provide wave-nrs and mem 
d=par(3); % diffusion param.; this and k-range usually only problem dep. things 
for i=1:kl %  now loop over wave-nr and compute Evals
    % HERE!!!!
    k=kv(i); K=[[k^(2*0.3) 0];[0 d*k^(2*0.3)]];  A=J-K; % Jac in Fourier space 
